...
// Rota para migrar dados locais para o Redis
      ...
      try {
        ...
        if (saved) {
          return NextResponse.json({ 
            success: true, 
            message: "Champion pool migrado com sucesso para o Redis!" 
          });
        } else {
          return NextResponse.json(
        ...
        console.error("[Migrate] Erro ao migrar champion-pool:", error);
        return NextResponse.json(
          { error: `Arquivo local não encontrado ou inválido: ${error}` },
          { status: 404 }
        );
      }
    }
    ...
    if (type === "compositions") {
      ...
      try {
        ...
        if (saved) {
          return NextResponse.json({ 
            success: true, 
            message: "Composições migradas com sucesso para o Redis!" 
          });
        } else {
          return NextResponse.json(
            { error: "Erro ao salvar no Redis. Verifique as variáveis de ambiente." },
        ...
        console.error("[Migrate] Erro ao migrar compositions:", error);
        return NextResponse.json(
          { error: `Arquivo local não encontrado ou inválido: ${error}` },
          { status: 404 }
        );
